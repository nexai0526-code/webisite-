...
foreach ($cands as $i=>$w) {
    ...
    // Holder gate: any whale holding memecoins ranks above all flippers
    $isHolder = $memeHolds > 0 ? 1 : 0;
    $score = ($isHolder*500) + ($memeHolds*40) + (min($memeVal,8000)/30) + ($alpha*0.3) + (min($totalVal,80000)/1000);
    $results[]=[
        'address'=>$addr,
        'label'=>$w['label']??('Whale'),
        'alpha'=>$alpha,
