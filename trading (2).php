...
--border:rgba(255,255,255,.05);--eth:#627EEA;--green:#00E676;--red:#FF4757;--amber:#FFD32A;--purple:#7C4DFF;
}
...
.s-val.eth{color:var(--eth)}.s-val.green{color:var(--green)}.s-val.amber{color:var(--amber)}.s-val.red{color:var(--red)}.s-val.purple{color:var(--purple)}
...
.coin-card,.profile-card{border-bottom:1px solid rgba(255,255,255,.03);transition:background .2s}
.coin-card:hover{background:rgba(255,255,255,.015)}
.profile-card:hover{background:rgba(124,77,255,.02)}
.cc-head,.pc-head{padding:14px 22px;display:grid;gap:14px;align-items:center;cursor:pointer}
...
.cc-info,.pc-info{min-width:0}
.cc-name,.pc-name{font-family:'Syne',sans-serif;font-size:15px;font-weight:700;display:flex;align-items:center;gap:8px;flex-wrap:wrap}
...
.cc-meta,.pc-meta{font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--text3);margin-top:3px}
...
.cc-toggle,.pc-toggle{font-size:18px;color:var(--text3);transition:transform .2s;padding:4px}
.coin-card.open .cc-toggle,.profile-card.open .pc-toggle{transform:rotate(180deg)}
.cc-body,.pc-body{display:none;padding:0 22px 18px;background:rgba(0,0,0,.15)}
.coin-card.open .cc-body,.profile-card.open .pc-body{display:block}
...
@media(max-width:768px){
  ...
  .wr-wallet{font-size:11px}.wr-meta{font-size:9px}.wr-net{font-size:11px}
  .coin-row{grid-template-columns:auto 1fr auto;gap:8px;padding:8px 10px}
  .cr-amount{display:none}
  .cr-symbol{font-size:11px}
}
</style>
</head>
<body>
...
<div class="container">
  ...
  <div class="tabs">
    ...
    <button class="tab" data-tab="whales">🐋 Smart Money Profiles</button>
  </div>
...
function renderCoins(){
  ...
  if(!coins || coins.length===0){
    document.getElementById('coinsList').innerHTML='<div class="empty">No coins match.</div>';
    return;
  }
  let h='';
  coins.forEach((c,idx)=>{
    ...
    if(c.whales && c.whales.length>0){
      ...
      whalesHtml='<div class="empty" style="padding:20px">No whales yet.</div>';
    }
    ...
    h+='<a href="https://gopluslabs.io/token-security/1/'+c.address+'" target="_blank" class="cc-action">🛡️ GOPLUS</a>';
    h+='</div></div></div>';
  });
...
function renderProfiles(){
  if(!allProfiles || allProfiles.length===0){
    document.getElementById('profilesList').innerHTML='<div class="empty">No active wallets found.</div>';
    return;
  }
  let h='';
  allProfiles.forEach((p,idx)=>{
    ...
    p.coins.forEach(c=>{
      ...
      const statusIcon=c.status==='HOLDING'?'💎':(c.status==='DUMPED'?'🔴':(c.status==='SOLD (TOP)'?'⚠️':'⇄'));
      coinsHtml+='<a href="'+c.dex_url+'" target="_blank" class="coin-row" style="border-left-color:'+c.status_color+'">';
      coinsHtml+='<div class="cr-icon" style="background:'+c.status_color+'20;color:'+c.status_color+'">'+statusIcon+'</div>';
      ...
      coinsHtml+='<div class="cr-meta">'+c.first_buy_age+' ago · '+c.count_buys+' buys / '+c.count_sells+' sells</div></div>';
    ...
    h+='<div class="pc-info">';
    h+='<div class="pc-name">'+p.wallet+' <span class="pc-label">'+p.discovered_from+'</span></div>';
    h+='<div class="pc-meta">'+p.coins_count+' coins · '+p.holding_count+' holding · '+p.exited_count+' exited · '+p.wins+'W/'+p.losses+'L</div>';
    h+='</div>';
    ...
    h+='<div class="pc-body">';
    h+='<div class="whales-title">📊 LAST 7 DAYS — COINS BOUGHT</div>';
    h+='<div class="coin-list">'+coinsHtml+'</div>';
    h+='<div class="cc-actions">';
    h+='<a href="'+p.explorer_url+'" target="_blank" class="cc-action">Ξ ETHERSCAN</a>';
    h+='<a href="https://debank.com/profile/'+p.wallet_full+'" target="_blank" class="cc-action">📊 DEBANK PORTFOLIO</a>';
    h+='</div></div></div>';
  });
